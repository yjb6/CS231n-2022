...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2*N):
          if(j!=k):
            denominator1+=torch.exp(sim(z_k,out[j])/tau)
          if(j!=k+N):
            denominator2+=torch.exp(sim(z_k_N,out[j])/tau)
        total_loss+=-torch.log(torch.exp(sim(z_k,z_k_N)/tau)/denominator1)-torch.log(torch.exp(sim(z_k_N,z_k)/tau)/denominator2)
...
def compute_sim_matrix(out):
    ...
    sim_matrix=out.matmul(out.T)/(torch.linalg.norm(out,dim=1,keepdim=True).matmul(torch.linalg.norm(out,dim=1,keepdim=True).T))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator=torch.exp(torch.cat([sim_pos, sim_pos], dim=0)/tau)

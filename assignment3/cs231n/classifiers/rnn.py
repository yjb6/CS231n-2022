...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type=='rnn':
          ...
          dx,dh0,grads['Wx'],grads['Wh'],grads['b']=lstm_backward(dh_out,cache_rnn)
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        pre_word=np.full((N,1),self._start)
        pre_h,_=affine_forward(features,W_proj, b_proj)
        pre_c=np.zeros(pre_h.shape)
        for t in range(max_length):
          
          x,_=word_embedding_forward(pre_word, W_embed)
          x=np.squeeze(x)
          if self.cell_type=='rnn':
            pre_h,_=rnn_step_forward(x, pre_h, Wx, Wh, b)
          else:
            pre_h,pre_c,_=lstm_step_forward(x, pre_h,pre_c, Wx, Wh, b)
          y,_=affine_forward(pre_h,W_vocab, b_vocab)
          pre_word=np.argmax(y,axis=1)
          captions[:,t]=pre_word
          pre_word=pre_word.reshape((N,1))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################

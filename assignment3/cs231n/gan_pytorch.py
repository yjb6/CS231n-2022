...
def sample_noise(batch_size, dim, seed=None):
    ...
    index=torch.rand(batch_size,dim)<0.5
    noise=torch.rand(batch_size,dim)*index
    noise+=-torch.rand(batch_size,dim)*(~index)
    return noise
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    label=torch.ones(logits_real.shape).type(dtype)
    label=torch.cat((label,torch.zeros(logits_real.shape).type(dtype)))
    label=label.squeeze()
    scores=torch.cat((logits_real,logits_fake))
    loss=bce_loss(scores,label)*2#其实两个loss应该分开算，因为对于D的loss，label为1的是前一项，label为0的是后一项
    #如果放在一起算会取个平均，所以要乘2
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return loss
...
def ls_generator_loss(scores_fake):
    ...
    N=scores_fake.shape[1]
    loss=torch.mean(0.5*torch.pow(scores_fake-1,2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return loss

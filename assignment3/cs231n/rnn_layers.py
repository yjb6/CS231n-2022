...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(x_.shape[0]):
      prev_h,cache_temp=rnn_step_forward(x_[t],prev_h,Wx,Wh,b)
...
def rnn_backward(dh, cache):
    ...
    dh_=dh.swapaxes(0,1)
    dh_next=None
    for T in range(dh_.shape[0]-1,-1,-1):
      dh_next=dh_[T]
      ...
      if dh0 is None:
        ...
        dh0+=dh_next
        dx_=np.pad(dx_,((0,dh_.shape[0]-1-T),(0,0),(0,0)),'constant')
        dx+=dx_
        dWx+=dWx_
        dWh+=dWh_
        db+=db_
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      x_=x[:,t,:].squeeze()
      h0,prev_c,cache_=lstm_step_forward(x_,h0, prev_c, Wx, Wh, b)
      h[:,t,:]=h0
      cache.append(cache_)
...
def lstm_backward(dh, cache):
    ...
    for T in range(dh_.shape[0]-1,-1,-1):
      dh_next=dh_[T]
      dc_next=np.zeros(dh_next.shape)
      dx_, dWx_, dWh_, db_=None,None,None,None
      for t in range(T,-1,-1):
        # dh_temp=dh_[t]
        # if dh_next is not None:
        #   print(1)
        #   dh_temp=dh_next
        dx_temp, dh_next,dc_next, dWx_temp, dWh_temp, db_temp=lstm_step_backward(dh_next,dc_next,cache[t])
        dx_temp=dx_temp[np.newaxis,:]
        if dx_ is None:
          dx_=dx_temp
          dWx_=dWx_temp
